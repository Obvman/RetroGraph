...
public:
    ...
    [[nodiscard]] Handle attach(Callback callback) const { return m_callbackList.append(callback); }

    void detach(Handle handle) const { m_callbackList.remove(handle); }

    void raise(Args... args) { m_callbackList(args...); }
