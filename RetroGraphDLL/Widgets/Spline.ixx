                        ...
                        spline_type type)
{
    ...
    if (type == linear) {
        ...
        if (m_left == spline::second_deriv) {
            ...
            A(0, 0) = 2.0f * (x[1] - x[0]);
            A(0, 1) = 1.0f * (x[1] - x[0]);
            rhs[0] = 3.0f * ((y[1] - y[0]) / (x[1] - x[0]) - m_left_value);
        } else {
            RGASSERT(false, "Spline");
        }
        if (m_right == spline::second_deriv) {
            ...
            A(n - 1, n - 1) = 2.0f * (x[n - 1] - x[n - 2]);
            A(n - 1, n - 2) = 1.0f * (x[n - 1] - x[n - 2]);
            rhs[n - 1] = 3.0f * (m_right_value - (y[n - 1] - y[n - 2]) / (x[n - 1] - x[n - 2]));
        } else {
            RGASSERT(false, "Spline");
        }
...
bool spline::make_monotonic()
{
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        if (avg == 0.0f && (m_b[i] != 0.0f || m_b[i + 1] != 0.0)) {
            modified = true;
            m_b[i] = 0.0;
            m_b[i + 1] = 0.0;
        } else if ((m_b[i] >= 0.0f && m_b[i + 1] >= 0.0f && avg > 0.0) ||
                   (m_b[i] <= 0.0f && m_b[i + 1] <= 0.0f && avg < 0.0)) {
...
void band_matrix::lu_decompose()
{
    ...
    for (int i = 0; i < this->dim(); i++) {
        RGASSERT(this->operator()(i, i) != 0.0, "Spline");
        this->saved_diag(i) = 1.0f / this->operator()(i, i);
